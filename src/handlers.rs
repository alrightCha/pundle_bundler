...
pub async fn sell_for_keypair(Json(payload): Json<UniqueSellRequest>) -> Json<SellResponse> {
...
pub async fn pay_recursive(Json(payload): Json<RecursivePayRequest>) -> Json<SellResponse> {
    let requester: String = payload.pubkey;
    let mint: String = payload.mint;
...
pub async fn bump_token(Json(payload): Json<BumpRequest>) -> Json<BumpResponse> {
    let mint_address = payload.mint_address;
    let lamports: u64 = payload.lamports;
    let mut bump: Bump = Bump::new(mint_address);

    let new_kp: Pubkey = bump.get_funding_pubkey();
    let clone_admin: Keypair = get_admin_keypair();
    let fund_ix = system_instruction::transfer(&clone_admin.pubkey(), &new_kp, lamports);
