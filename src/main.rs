...
use solana::helper::{pack_instructions, PackedTransaction};
...
fn get_signers(signers_map: HashMap<Pubkey, Keypair>, instructions: Vec<Instruction>) -> Vec<Keypair> {
    ...
    for signer in unique_signers {
        if let Some(kp) = signers_map.get(&signer) {
            tx_signers.push(kp.insecure_clone());
        } else {
...
async fn main() {
    ...
    let mut all_keypairs: Vec<&Keypair> = Vec::new();
